<script>
   ...
   // const link = (d) => `https://github.com/prefuse/Flare/blob/master/flare/src/${d.id.replace(/\./g, '/')}.as`;
    ...
    let colorScale = scaleOrdinal(groups).domain([0, 1]).range(['#964CAD', '#F8553D'])
    $:{
      ...
      groups = new InternSet(groups);
      colorScale = scaleOrdinal(groups).domain([0, 1]).range(['#964CAD', '#F8553D'])
    }
...
<main class="circles">
  <div class = "violence_grid"> 
    ...
    style="left: {recorded_mouse_position.x + 2}px; top: {recorded_mouse_position.y + 2}px">
    {#if hovered !== -1}
